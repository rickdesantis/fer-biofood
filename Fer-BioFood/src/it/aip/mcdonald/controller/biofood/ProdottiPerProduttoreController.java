...
public class ProdottiPerProduttoreController extends Controller {
    ...
    public Navigation run() throws Exception {
        ...
        for (Prodotto p : list) {
            ...
            if (tmp.indexOf("+") > -1)
                tmp = tmp.replaceAll("+", " ");
            if (p.getProduttoreRef().getModel().getNome().equals(tmp))
                prodotti.add(p.getNome());
        }
        
        requestScope("prodotti", prodotti);
        
        return forward("prodottiPerProduttore.jsp");
    }
}
